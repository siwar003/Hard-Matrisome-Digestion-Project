---
title: "Heat-Based Extraction and Enrichment of Hard Matrisome Proteins from Collagen-Rich Tissue"
author: ""
output:
  github_document:
    toc: true
...
> **Figure 1.** Photograph of dried tendons (A) before and (B) after heating in canola oil at 180 °C, demonstrating successful expansion with minimal browning.

Heat-treated tendons were then incubated in 1.5 mL chloroform under mild agitation for 45--60 minutes at room temperature to delipidate the sample and remove the bulk of surface oil. The tendons were then removed from the chloroform wash and incubated in water at either room temperature or 80 °C for 1 hour or 48 hours, according to their assigned conditions (Table 1). Collagenase-treated samples were incubated with 500 µL of 2 mg/mL (0.2% w/v) collagenase type I, to allocate approximately 1 mg of collagenase for ~10 mg of tissue for 1 hour at 45 °C^10,11^. The PBS buffer was also supplemented with 0.9 mM CaCl~2~ to stabilize collagenase activity. Samples were then centrifuged after treatment and protein precipitation using chloroform and methanol was performed.

### Table 1. Experimental conditions

| Code | Condition label | Pre-heat (180 °C) | Solvent / enzyme | Incubation temp | Duration |
|:----:|:----------------|:------------------:|:-----------------|:---------------:|:--------:|
...
> **Figure 2.** Solubility comparison showing percentage of original dry mass solubilized for each treatment. Pre-heat treatments (red) vs. no pre-heat (blue).

## PSM enrichment analysis

After the supernatants were isolated, they were digested overnight with trypsin, and the samples were analyzed by mass spectrometry. From mass spectrometry data, the total peptide spectrum matches (PSMs) and PSMs attributed to Matrisome proteins were plotted across all conditions (Figure 3a). The number of PSMs represents the number of times a peptide in each condition was matched to a spectrum during MS2, and it provides a relative measure of sample depth.
...
The following code generates the stacked bar chart shown in Figure 3a (see [`scripts/psm_enrichment_stacked_bar.R`](scripts/psm_enrichment_stacked_bar.R)):

```{r fig3a-psm-stacked-bar, echo=TRUE, eval=FALSE}
# Key plotting code from scripts/psm_enrichment_stacked_bar.R
data <- data.frame(
...
data_long <- data %>%
  mutate(Non_matrisome = Total - Matrisome) %>%
  pivot_longer(cols = c(Non_matrisome, Matrisome),
               names_to = "Fraction", values_to = "PSMs")

ggplot(data_long, aes(x = Condition, y = PSMs, fill = interaction(Group, Fraction))) +
  geom_bar(stat = "identity", position = "stack", width = 0.65) +
  labs(title = "Total PSMs with Matrisome Fraction per Condition",
       y = "PSMs") +
  theme_classic(base_size = 13)
```

> **Figure 3.** (a) Stacked bar chart showing total and Matrisome PSMs per condition. (b) Percentage of protein signal originating from Matrisome proteins by Razor intensity in each treatment.
...
The intensity enrichment bar chart is generated by [`scripts/matrisome_intensity_enrichment.R`](scripts/matrisome_intensity_enrichment.R):

```{r fig3b-intensity, echo=TRUE, eval=FALSE}
# Core plotting logic from scripts/matrisome_intensity_enrichment.R
# Reads per-sample TSV files, classifies proteins as Matrisome/non-Matrisome
# using NABA_MATRISOME GMT gene set, then calculates:
#   normalized_intensity = sample_intensity / sample_total_intensity
#   percent_matrisome    = 100 * sum(normalized_intensity[is_matrisome])
#
# Final plot: condition-level bar chart with SD error bars and individual
# sample points overlaid.
# source("scripts/matrisome_intensity_enrichment.R")
```
...
```{r table3-coverage, echo=FALSE}
coverage <- read.csv("condition_coverage_summary_by_subset.csv")

coverage_table <- coverage %>%
  filter(subset_label %in% c("Core Matrisome", "Matrisome Associated",
                               "Collagens", "Proteoglycans", "ECM Glycoproteins")) %>%
  select(subset_label, condition_label, median_coverage) %>%
  mutate(median_coverage = round(median_coverage, 1)) %>%
  pivot_wider(names_from = condition_label, values_from = median_coverage)

knitr::kable(coverage_table, caption = "Median sequence coverage (%) across conditions")
```
...
The sequence coverage boxplots are generated by [`scripts/matrisome_coverage_boxplots.R`](scripts/matrisome_coverage_boxplots.R):

```{r fig4-seqcov, echo=TRUE, eval=FALSE}
# Core plotting logic from scripts/matrisome_coverage_boxplots.R
# For each NABA subset (GMT file), the script:
#   1. Reads per-sample TSV files and filters decoys/contaminants
#   2. Joins protein Gene symbols to NABA subset membership
#   3. Plots boxplots of sequence coverage by condition
#
# Example for one subset:
# ggplot(plot_df, aes(x = condition_label, y = coverage, fill = condition_label)) +
#   geom_boxplot(width = 0.62) +
#   scale_fill_manual(values = colour_map) +
#   labs(title = paste0(subset_label, " Sequence Coverage"),
#        y = "Sequence coverage (%)") +
#   theme_bw(base_size = 14)
#
# source("scripts/matrisome_coverage_boxplots.R")
```

> **Figure 4.** Sequence coverage of (a) core and (b) non-core Matrisome proteins across different conditions, including (c) collagens, (d) proteoglycans, and (e) glycoproteins.
...
> **Figure 5.** Sequence coverage plots including the cold water controls for (a) core and (b) non-core Matrisome proteins across different conditions, including (c) collagens, (d) proteoglycans, (e) Matrisome-associated proteins, and (f) glycoproteins.

## NABA subset gene matching summary

### Table 4. Genes matched per NABA Matrisome subset

```{r table4-gene-match, echo=FALSE}
gene_match <- read.csv("subset_gene_match_summary.csv")

gene_display <- gene_match %>%
  select(Subset = subset_label,
         `Matched genes` = matched_genes,
         `Matched proteins` = matched_proteins,
         `Genes in subset` = genes_in_subset) %>%
  mutate(`Match rate (%)` = round(`Matched genes` / `Genes in subset` * 100, 1))

knitr::kable(gene_display, caption = "NABA subset gene matching across all conditions")
```
...
1. Add 4x sample volume of ice-cold methanol (600 µL). Vortex 10 s.
2. Add 1x sample volume of cold chloroform (150 µL). Vortex 10 s.
3. Add 3x sample volume of water (450 µL). Vortex 10 s, then centrifuge 1 min at 14,000 x g. A protein wafer will appear at the interphase.
4. Carefully remove and discard the upper aqueous phase without disturbing the wafer.
5. Add 4x sample volume of cold methanol (600 µL). Vortex and centrifuge 2 min at 14,000 x g to pellet proteins. Discard supernatant.
...
| Buffer | Composition |
|:-------|:------------|
| Buffer C (Cytosolic) | 10 mM HEPES (pH 7.9); 10 mM KCl; 1.5 mM MgCl~2~; 0.34 M sucrose; 10% glycerol |
| Buffer W (Wash) | 10 mM HEPES (pH 7.9); 10 mM KCl; 1.5 mM MgCl~2~; 0.34 M sucrose; 10% glycerol |
| Buffer N (Nuclear) | 20 mM HEPES (pH 7.9); 420 mM NaCl; 1.5 mM MgCl~2~; 0.2 mM EDTA; 20% glycerol; benzonase |
| Buffer M (Membrane) | 10 mM HEPES (pH 7.9); 10 mM KCl; 1.5 mM MgCl~2~; 0.34 M sucrose; 10% glycerol; 1% NP-40 |
| Buffer CS (Cytoskeletal) | 10 mM HEPES (pH 6.8); 100 mM NaCl; 1 mM MgCl~2~; 0.3 M sucrose; 1% SDS |
...
2. The pellet was resuspended in 45 µL of Buffer N supplemented with benzonase for nuclear protein extraction. Samples were placed on a tube rotator for 30 min at 4 °C, then centrifuged for 30 min at 4 °C. The centrifugation step was repeated once more, and the supernatants were discarded.
3. The pellet was resuspended in 30 µL of Buffer M for membrane protein extraction. Samples were placed on a tube rotator for 30 min at 4 °C, then centrifuged for 30 min at 4 °C. The supernatant was discarded.
4. The pellet was resuspended in 60 µL of Buffer CS for cytoskeletal protein extraction. Samples were placed on a tube rotator for 30 min at 4 °C, then centrifuged for 30 min at 4 °C. The supernatant was removed, and the pellet was resuspended in 45 µL of Buffer C, rotated for 20 min at 4 °C, centrifuged for 20 min at 4 °C, and the supernatant removed.
